...
test.describe("TaskFlow 真实冒烟测试", () => {
  ...
  test("登录后完成创建、更新、删除任务的完整链路", async ({ page }) => {
    test.skip(!email || !password, "缺少 TASKFLOW_E2E_EMAIL 或 TASKFLOW_E2E_PASSWORD 环境变量，已跳过真实冒烟测试。");
    ...
    await expect(page.getByRole("button", { name: "继续", exact: true })).toBeVisible({ timeout: 30_000 });
    ...
    await page.getByRole("textbox", { name: "备注" }).fill("真实 Appwrite 冒烟测试任务");
    await page.getByRole("button", { name: "创建任务", exact: true }).click();

    const taskTable = page.getByRole("table", { name: "任务列表" });
    await expect(taskTable.getByText(title, { exact: true })).toBeVisible({ timeout: 15_000 });
    ...
    await expect(taskTable.getByText(title, { exact: true })).toHaveCount(0);
  });
});

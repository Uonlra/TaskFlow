...
TaskFlow 是一个偏个人工作台风格的任务管理项目，用来把任务记录、筛选排序、截止提醒和进度看板放到同一个应用里。
...
- Dashboard 支持今天、本周、全部三个观察范围，并同步到 URL
...
- 页面状态尽量可恢复：Dashboard 范围和任务筛选都放进 URL。
- 任务不只是列表：截止日期、优先级、状态和标签会一起参与排序与提示。
- 后端可选但体验不断：没有 Appwrite 时仍然能看完整界面和主要交互。
- 组件按页面领域拆分：认证、任务、看板、布局和设置各自独立，后续扩展会比较清楚。

      ...
      <aside className="auth-brand-panel">
        <div className="auth-brand-copy">
          ...
          <p className="auth-brand-description">
            从今天要做什么，到下一项该推进什么，都在同一个任务工作台里保持清楚。
          </p>
        </div>

        <div className={isReady ? "auth-product-surface auth-product-surface--ready" : "auth-product-surface"} aria-label="任务工作台预览">
          <nav className="auth-preview-nav" aria-label="工作台功能">
            <p className="auth-preview-nav__label">工作台</p>
            <ul>
              {previewNavigation.map((item) => (
                <li key={item.kind} className={item.active ? "auth-preview-nav__item auth-preview-nav__item--active" : "auth-preview-nav__item"}>
            ...
            <div className="auth-preview-nav__profile">
              ...
              <div><strong>{isReady ? displayName : "等待登录"}</strong><small>{previewMessage}</small></div>
            </div>
          </nav>

          <div className="auth-preview-main">
            <header className="auth-preview-main__header">
              <div><p>总览</p><strong>{isReady ? displayName + "的工作台" : "今天的工作台"}</strong></div>
              <span className={isReady ? "auth-preview-connection auth-preview-connection--ready" : "auth-preview-connection"}>
            ...
            <section className="auth-preview-focus">
              ...
              <span>
                {isReady
                  ? "现在可以查看任务状态、截止提醒和进度记录。"
                  : "任务状态、截止提醒和进度记录会在此处同步。"}
              </span>
            </section>
            <div className="auth-preview-status-grid">
              <div><span>全部任务</span><strong>{isReady ? summary.totalCount : "等待同步"}</strong></div>
              <div><span>待处理</span><strong>{isReady ? summary.todoCount + summary.inProgressCount : "等待同步"}</strong></div>
              <div><span>已完成</span><strong>{isReady ? summary.doneCount : "等待同步"}</strong></div>
              <div><span>逾期提醒</span><strong>{isReady ? summary.overdueCount : "等待同步"}</strong></div>
            </div>
            <section className="auth-preview-activity">
              <div className="auth-preview-activity__heading"><span>{isReady ? "优先查看" : "最近动态"}</span><small>{isReady ? "当前任务" : "登录后显示"}</small></div>
              {isReady && summary.focusTasks.length > 0 ? summary.focusTasks.slice(0, 2).map((task) => (
                <div className="auth-preview-activity__row" key={task.id}><i aria-hidden="true" /><span>{task.title}</span><small>{task.dueLabel}</small></div>
              )) : (
                <div className="auth-preview-activity__row"><i aria-hidden="true" /><span>{isReady ? "还没有待处理任务" : "你的任务记录会出现在这里"}</span></div>
              )}
            </section>
          </div>
        </div>
